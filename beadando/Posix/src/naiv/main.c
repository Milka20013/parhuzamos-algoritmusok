...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < number_of_threads; i++)
    {
        ...
        thread_params[i].start_index = (numberOfLetters / number_of_threads) * i;
        thread_params[i].end_index = (numberOfLetters / number_of_threads) * (i + 1) + str->length + 1;
        thread_params[i].multiplier = multiplier;
    }
    thread_params[number_of_threads - 1].end_index = numberOfLetters - str->length;

    clock_t start = clock();
    for (int i = 0; i < number_of_threads; i++)
    {
        pthread_create(&threads[i], NULL, search_string_naiv, (void *)&(thread_params[i]));
    }
